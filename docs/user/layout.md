...
parent as either one moves or resizes, leaves the screen with it when the strip
scrolls, dragging it drags the parent, and while it is open the parent is shaded
and gets neither focus nor pointer input, both go to the dialog. A blocked
window can still be dragged directly, as if with Mod held. With the dialog
focused, focus and column actions such as `window-toggle-floating` or
`window-focus-left` act on that window, and focusing the dialog scrolls the
window into view. A dialog is modal when it says so through xdg-dialog-v1, as
GTK 4 and Qt 6 do; such a dialog also blocks every other window its application
had open on the workspace before it, the way the toolkit treats it. A dialog
another process attached through xdg-foreign, as portals do with their file
chooser, is modal too, and blocks only its parent and the dialogs the parent had
open.
