...
namespace umbriel {
  ...
  void View::applyDynamicRules(const ResolvedWindowRule* resolved) {
    ...
    const bool chromeChanged = m_decoration.applyRule(rule);
    ...
    if (chromeChanged) {
      // A rule that names a decoration key redraws the ring, the surface radius, and the shadow. The layout spacing a
      // ring reserves stays global, so no arrange is needed here.
      updateBorderGeometry();
      applyCornerRadius();
      updateShadow();
    }
